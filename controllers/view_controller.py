...
def consulta():
    ...
    registros = filter_records(municipio=municipio, fecha=fecha_formateada)
    return render_template("consulta.html", registros=registros)
...
def comparar():
    ...
    municipio = request.form.get("municipio", "").strip()
    fecha_html = request.form.get("fecha", "").strip() # <--- Capturamos la fecha
    ...
    # LLAMADA CON LOS DOS ARGUMENTOS
